...
# Script from https://github.com/google-research/deduplicate-text-datasets/tree/master
...
for jobstart in range(0, total_jobs, jobs_at_once):
    ...
    for i in range(jobstart, jobstart + jobs_at_once):
        ...
        cmd = "./target/debug/dedup_dataset make-part --data-file %s --start-byte %d --end-byte %d" % (
        sys.argv[1], s, e)
        started.append((s, e))
        print(cmd)
...
while True:
    files = ["%s.part.%d-%d" % ('tmp/' + sys.argv[1].split('/')[-1], s, e) for s, e in started]
    ...
    for x, (s, e) in zip(files, started):
        ...
        if not os.path.exists(x) or not os.path.exists(
                x + ".table.bin") or size_table == 0 or size_data * FACT != size_table:
            cmd = "./target/debug/dedup_dataset make-part --data-file %s --start-byte %d --end-byte %d" % (
            sys.argv[1], s, e)
            print(cmd)
            wait.append(os.popen(cmd))
...
print("./target/debug/dedup_dataset merge --output-file %s --suffix-path %s --num-threads %d" % (
"tmp/out.table.bin", torun, mp.cpu_count()))
pipe = os.popen("./target/debug/dedup_dataset merge --output-file %s --suffix-path %s --num-threads %d" % (
"tmp/out.table.bin", torun, mp.cpu_count()))
output = pipe.read()
